...
<title>您搜索的【<?php echo $keyword; ?>】结果如下-澈逆凡博客</title>
            ...
            <div class="row">
                <div class="article col-xs-12 col-sm-8 col-md-8">
                    ...
                    <div class="ajax-load-box posts-con" id="list001">
                        <?php $list_return = $this->list_tag("action=search module=MOD_DIR id=$searchid total=$sototal order=$params[order] catid=$catid page=1 pagesize=10 urlrule=$urlrule"); if ($list_return && is_array($list_return)) extract($list_return, EXTR_OVERWRITE); $count=dr_count($return); if (is_array($return)) { $key=-1; foreach ($return as $t) { $key++; $is_first=$key==0 ? 1 : 0;$is_last=$count==$key+1 ? 1 : 0; ?>
