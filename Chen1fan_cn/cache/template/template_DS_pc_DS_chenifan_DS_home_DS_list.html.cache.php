            ...
            <div class="row">
                <div class="article col-xs-12 col-sm-8 col-md-8">
                    ...
                    <div class="ajax-load-box posts-con" id="list001">
                        <?php $list_return = $this->list_tag("action=module module=article catid=$cat[id] page=1 num=10"); if ($list_return && is_array($list_return)) extract($list_return, EXTR_OVERWRITE); $count=dr_count($return); if (is_array($return)) { $key=-1; foreach ($return as $t) { $key++; $is_first=$key==0 ? 1 : 0;$is_last=$count==$key+1 ? 1 : 0; ?>
                        <div class="ajax-load-con content excerpt-one">
                            <div class="content-box posts-image-box">
                                <div class="posts-default-title">
                                     <div class="post-entry-categories">
                                <?php $list_return_rs = $this->list_tag("action=tag tag=$t[keywords]  num=5   return=rs"); if ($list_return_rs && is_array($list_return_rs)) extract($list_return_rs, EXTR_OVERWRITE); $count_rs=dr_count($return_rs); if (is_array($return_rs)) { $key_rs=-1;  foreach ($return_rs as $rs) { $key_rs++; $is_first=$key_rs==0 ? 1 : 0;$is_last=$count_rs==$key_rs+1 ? 1 : 0; ?>
...
<?php } } ?>
                                ...
                                <p class="focus">
                                    <a href="<?php echo $t['url']; ?>" target="_blank" class="thumbnail">
                                        <span class='item'><span class='thumb-span'><img src="<?php if ($t['thumb']) {  echo dr_get_file($t['thumb']);  } else { ?>/pic/<?php echo $t['title']; ?>.jpg<?php } ?>" class='thumbnail lazy'></span></span>
                                    </a>
                                </p>
