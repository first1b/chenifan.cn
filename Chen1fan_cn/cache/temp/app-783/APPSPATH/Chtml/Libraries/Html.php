...
class Html {
    ...
    public function _Create_Show_Html($ci, $mid, $id, $page = 0) {
        ...
        if (!$hfile) {
            if (strpos($data['url'], 'index.php?')!==false) {
                return dr_return_data(0, '地址【'.$data['url'].'】是动态，请更新内容URL地址为静态模式');
            }
    ...
    public function _Category_Html_File($ci, $mid) {
        ...
        $this->_Create_Category_Html($ci, $mid, intval(\Phpcmf\Service::L('input')->get('id')));
        exit;
    ...
    public function _Index_Html($ci) {
        ...
        } elseif ($ci->module['setting']['search']['indexsync']) {
    ...
    public function del_auth_data($name) {
        $path = WRITEPATH.'chtml/';
        $file = $path.SITE_ID.'-'.$name.'.json';
        @unlink($file);
    }
