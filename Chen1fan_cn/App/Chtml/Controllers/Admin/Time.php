...
class Time extends \Phpcmf\Table
{
    ...
    public function ct_index() {
        ...
        if (!$page) {
            $cts = \Phpcmf\Service::M()->table($data['siteid'].'_'.$data['mid'])->counts();
            if (!$cts) {
        ...
        $ids = '';
        $data = \Phpcmf\Service::M()->db->table($data['siteid'].'_'.$data['mid'])->select('id')->limit($psize, $psize * ($page - 1))->orderBy('id DESC')->get()->getResultArray();
        foreach ($data as $row) {
            $ids.= ','.$row['id'];
        }
        $ids = trim($ids, ',');
