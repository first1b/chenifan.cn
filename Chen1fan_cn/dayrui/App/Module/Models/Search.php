    ...
    public function get_data() {
        ...
        if (!$data) {
                ...
                if ($cat_field) {
                    ...
                    foreach ($cat_field as $name) {
                        if (isset($this->get[$name]) && strlen($this->get[$name])) {
                            $more = 1;
                            $r = $this->_where($table_more, $name, $this->get[$name], $this->module['category_data_field'][$name], $is_like);
                            if ($r) {
                                $more_where[] = $r;
            ...
            foreach ($mod_field as $name => $field) {
                ...
                if (isset($this->get[$name]) && strlen($this->get[$name])) {
                    $r = $this->_where($table, $name, $this->get[$name], $field, $is_like);
                    if ($r) {
                        $where[$name] = $r;
                        $param_new[$name] = $this->get[$name];
                    }
                ...
                if (\Phpcmf\Service::C()->member_cache['field']) {
                    foreach (\Phpcmf\Service::C()->member_cache['field'] as $name => $field) {
                        ...
                        if (!isset($mod_field[$name]) && isset($this->get[$name]) && strlen($this->get[$name])) {
                            $r = $this->_where($this->dbprefix('member_data'), $name, $this->get[$name], $field, $is_like);
                            if ($r) {
                                $member_where[] = $r;
                                $param_new[$name] = $this->get[$name];
                            }
