...
class Module extends \Phpcmf\Common {
    ...
    public function _Category($catid = 0, $catdir = null, $page = 1, $rt = 0) {
                    ...
                    if ($i != $catid && $this->module['category'][$i]['show']
                        ...
                        if (!$rt) {
                            ...
                            if (defined('SC_HTML_FILE')) {
                                ...
                                \Phpcmf\Service::L('Router')->redirect($url);
                                exit;
                            }
                        }
                        // 初始化模块
        ...
        // 判断内容唯一性
        !$rt && \Phpcmf\Service::L('Router')->is_redirect_url($page > 1 ? dr_url_prefix(dr_module_category_url($this->module, $category, $page), $this->module['dirname']) : $category['url'], 0, 1);
    ...
    public function _Show($id = 0, $param = [], $page = 1, $rt = 0) {
        ...
        // 判断内容唯一性
        !$rt && \Phpcmf\Service::L('Router')->is_redirect_url(dr_url_prefix($page > 1 ? dr_module_show_url($this->module, $data, $page) : $data['url'], $this->module['dirname']), 0, 1);
