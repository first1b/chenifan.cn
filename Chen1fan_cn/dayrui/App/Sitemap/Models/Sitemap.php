...
class Sitemap extends \Phpcmf\Model {
    ...
    public function tag_txt() {
        ...
        if (isset($config['autotime']) && $config['autotime']) {
            ...
            $cache_file = WRITEPATH.'app/sitemap/tag_'.DOMAIN_NAME.'.txt';
    ...
    public function sitemap_xml() {
        ...
        if (isset($config['autotime']) && $config['autotime']) {
            dr_mkdirs(WRITEPATH.'app/sitemap/');
            $cache_file = WRITEPATH.'app/sitemap/xml_'.$module.'_'.DOMAIN_NAME.'.txt';
            $time = filectime($cache_file);
            if (SYS_TIME - $time < 3600*intval($config['autotime'])) {
                // 没有超时，不生成
                if (is_file($cache_file)) {
    ...
    public function sitemap_txt() {
        ...
        if (isset($config['autotime']) && $config['autotime']) {
            dr_mkdirs(WRITEPATH.'app/sitemap/');
            $cache_file = WRITEPATH.'app/sitemap/txt_'.$module.'_'.DOMAIN_NAME.'.txt';
            $time = filectime($cache_file);
            if (SYS_TIME - $time < 3600*intval($config['autotime'])) {
                // 没有超时，不生成
                if (is_file($cache_file)) {
